Config
   { font                = "xft:JetBrains Mono:pixelsize=12:antialias=true:autohint=true"
   ...
   , persistent         = True
   , hideOnStart        = False
   , iconRoot           = "/home/xeera/.config/xmobar/xpm/"  				-- default: "."
   ...
   , commands           = [ Run Date "<fn=2>\xf017</fn> %b %d %Y  %H:%M:%S" "date" 10
	       ...
	       , sepChar = "%"
	       , alignSep = "}{"
       , template = " <icon=arch.xpm/><fc=#689d6a>  <fn=2></fn></fc>  %UnsafeStdinReader% }{ <fc=#427b58><fn=3></fn> %uname%</fc>   <fc=#b57614>%cpu%</fc>   <fc=#af3a03>%memory%</fc>   <fc=#076678>%disku%</fc>   <fc=#797403>%battery%</fc>   <fc=#8f3f71>%uptime%</fc>   %default:Master%   <fc=#d65d0e>%date%</fc>  "
       }
