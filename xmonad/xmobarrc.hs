Config {
       ...
       , commands = [ Run Date "<fn=2>\xf017</fn> %b %d %Y  %H:%M:%S" "date" 10
                    ...
                    , Run DynNetwork ["-t","<fc=#458588><fn=2></fn></fc> <rx>, <fc=#b16286><fn=2></fn></fc> <tx>"
                                     ,"-H","200"
                                   ,"-L","10"
                                     ,"-h","#bbc2cf"
                                     ,"-l","#bbc2cf"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " <icon=arch.xpm/><fc=#fe8019> <fn=2></fn> </fc>%UnsafeStdinReader% }{ <fc=#427b58><fn=3></fn> %uname%</fc>    <fc=#b57614>%cpu%</fc>   <fc=#af3a03>%memory%</fc>   <fc=#076678>%disku%</fc>   <fc=#797403>%battery%</fc>   <fc=#8f3f71>%uptime%</fc>  %dynnetwork%   <fc=#d65d0e>%date%</fc>  "
       }
